...
const formSchema = z.object({
    ...
    if(data.activities.length > 0) {
        data.activities.forEach((activity) => {
            if(activity.startTimestamp >= activity.endTimestamp) {
                return { message: "Activity start date must be before end date", path: ["activities", "startTimestamp", "endTimestamp"] }
            }
        })
    }
    return true;
    ...
    if(data.activities.length > 0) {
        data.activities.forEach((activity) => {
            if(activity.location.latitude < -90 || activity.location.latitude > 90) {
                return { message: "Latitude must be between -90 and 90", path: ["activities", "location", "latitude"] }
            }
        })
    }
    return true;
    ...
    if(data.activities.length > 0) {
        data.activities.forEach((activity) => {
            if(activity.location.longitude < -180 || activity.location.longitude > 180) {
                return { message: "Longitude must be between -180 and 180", path: ["activities", "location", "longitude"] }
            }
        })
    }
    return true;
